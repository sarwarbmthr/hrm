...
def visa_expiry():
    """
    This method is used to find employees with visa expiring within next 30 days
    """
    today = datetime.now().date()
...
def get_employees_visa_expiry(request):
    """
    Render all upcoming visa expiry employee details for the dashboard.
    """
    ...
    visa_expiries = [
        {
            ...
            "name": f"{emp.employee_first_name} {emp.employee_last_name}",
            "visa_expiry_date": emp.visa_expire_date.strftime("%d %b"),
            "daysUntilExpiry": (
                _("Today")
                if emp.days_until_visa_expiry == 0
                else (
                    _("Tomorrow")
                    if emp.days_until_visa_expiry == 1
                    else f"In {emp.days_until_visa_expiry} Days"
                )
            ),
            "department": (
                emp.get_department().department if emp.get_department() else ""
...
def dashboard(request):
    ...
    # Format visa expiry data for template
    default_avatar_url = "https://ui-avatars.com/api/?background=random&name="
    upcoming_visa_expiries = [
        {
            "profile": (
                emp.get_avatar()
                if hasattr(emp, "get_avatar")
...
@login_required
def visa_expiry_count(request):
    """
    Return count of employees with visa expiring within next 30 days
    """
    employees_with_visa_expiry = visa_expiry()
    return HttpResponse(employees_with_visa_expiry.count())
